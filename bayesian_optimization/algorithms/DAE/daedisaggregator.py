...
class DAEDisaggregator(Disaggregator):
    ...
    def __init__(self, sequence_length, patience, optimizer, learning_rate, loss):
        ...
        self.loss = loss
        self.model = self._create_model(self.sequence_length, self.optimizer, self.learning_rate, self.loss)

        # For keeping track of number of earlystop epoch
    ...
    def train_on_chunk(self, mainchunk, meterchunk, epochs, batch_size):
        ...
        # Use Validation validation_split=0.2
        self.model.fit(X_batch, Y_batch, batch_size=batch_size, epochs=epochs, shuffle=True, validation_split=0.2, callbacks=callbacks_list)
        # self.model.fit(X_batch, Y_batch, batch_size=batch_size, epochs=epochs, shuffle=True)

        # For keeping track of number of earlystop epoch
        self.stopped_epoch = earlystop.stopped_epoch

    def train_across_buildings(self, mainlist, meterlist, epochs, batch_size=128, **load_kwargs):
    ...
    def _create_model(self, sequence_len, optimizer, learning_rate, loss):
